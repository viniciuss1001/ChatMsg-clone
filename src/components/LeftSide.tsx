...
function timeConverter(UNIX_timestamp: Timestamp): string {
     const newDate = new Date(UNIX_timestamp.seconds * 1000)

     return (
          newDate.getHours().toLocaleString('pt-BR', {
               minimumIntegerDigits: 2,
               useGrouping: false,
          })
          + " : " +
          newDate.getMinutes().toLocaleString('pt-BR', {
               minimumIntegerDigits: 2,
               useGrouping: false,
          })
     )
...
type Props = {
     ...
     userId?: string
     displayName: string
     selectedChatRoom: (a: string) => void
     setSelectedChatRoom: (b: string) => void
}
...
export default memo(function LeftSide({
     ...
     return (
          ...
          >
               <div className='sticky bg-white top-0'>
                    <div className='flex items-center justify-between pt-4'>
                         <div
                              ...
                              <img src={picture || emptyProfile} alt="profile"
                                   className='w-12 h-12 rounded-full pointer-events-none object-cover'
                              />
                         </div>
                         <p className='font-semibold text-lg'>Mensagens</p>
                         <p
                    ...
                    <div
                         className='mt-6 flex space-x-2 border-2 items-center bg-white px-3 py-2 rounded-lg'
                    >
                         <svg
                              xmlns="http://www.w3.org/2000/svg"
                              className="h-6 w-6"
               ...
               <div
               className='overflow-auto scrollbar-none mt-2'
               >
                    {!chatRooms && 
                    [1,2,3,4].map((item)=>(
                         <div
                         key={item}
                         className='h-14 cursor-pointer w-full mt-2 items-center justify-between rounded-md px-2 py1 bg-gray-100 animate-pulse'
                         >
