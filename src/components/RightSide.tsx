...
export default memo(function RightSide({
     chatRoomId, firstName, lastName, setOpen, picture, userId
}: RSideProps) {
     ...
     useEffect(() => {
          if (chatRoomId) {
               setSelectedFriend({ email: '', firstName: '', lastName: '', picture: '' })
               getDoc(doc(db, 'chats', chatRoomId)).then((res) => {
                    if (res.exists()) {
                         getDoc(
                              doc(db, 'users', res.data().userIds[0] === userId
                                   ? res.data().userIds[1]
                                   : res.data().userIds[0]
                              )
                         ).then((res) => {
     ...
     return (
          ...
          >
               <ChatHeader
                    ...
                    firstName={selectedFriend?.firstName || "_____"}
                    lastName={selectedFriend?.lastName || "______"}
